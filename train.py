...
class Trainer(object):
  ...
  def run(self):
    ...
    for i in tqdm(xrange(FLAGS.num_episodes)):
      ...
      if i % FLAGS.validation_frequency == 0:
        ...
        tf.summary.scalar('accuracy', correct_by_shot[1])
        ...
        #print(np.mean(correct_by_shot[1]))
        summary_1shot = tf.Summary(value=[tf.Summary.Value(tag='accuracy',
                                                     simple_value=np.mean(correct_by_shot[1]))])
        train_writer_1shot.add_summary(summary_1shot, i)
        summary_5shot = tf.Summary(value=[tf.Summary.Value(tag='accuracy',
                                                           simple_value=np.mean(correct_by_shot[5]))])
        train_writer_5shot.add_summary(summary_5shot, i)
      ...
      #                                    sess.graph)
      #est_writer = tf.summary.FileWriter('log/test')
