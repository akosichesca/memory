...
class Memory(object):
  ...
  def query(self, query_vec, intended_output, use_recent_idx=True):
    ...
    normalized_query = myutils.reduce_precision(tf.nn.l2_normalize(query_vec, dim=1))
    ...
    fetched_keys_upd = myutils.reduce_precision(tf.nn.l2_normalize(fetched_keys_upd, dim=1))
