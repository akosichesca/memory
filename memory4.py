...
class Memory(object):
  ...
  def __init__(self, key_dim, memory_size, vocab_size, precision_bits,
  ...
  def get_hint_pool_idxs(self, normalized_query, batch_size):
    ...
    with tf.device(self.nn_device):
      ...
      qkeys2 = myutils.reduce_precision(qkeys, precision_bits=self.precision_bits)
      my_mem_keys2 = myutils.reduce_precision(self.mem_keys, precision_bits=self.precision_bits)
  ...
  def query(self, query_vec, intended_output, use_recent_idx=True):
    ...
    hint_pool_idxs = self.get_hint_pool_idxs(normalized_query, batch_size)
    ...
    with tf.device(self.var_cache_device):
      with tf.device(self.var_cache_device):
        ...
        qkeys2 = myutils.reduce_precision(qkeys, precision_bits=self.precision_bits)
        my_mem_keys2 = myutils.reduce_precision(my_mem_keys, precision_bits=self.precision_bits)
